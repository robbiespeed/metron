...
export interface Orb extends Particle {
  ...
  /**
   * Runs the callback when tracked watchables change
   */
  start (): void;
  stop (): void;
  clearWatched (): void;
...
export function createOrb (options?: OrbOptions): Orb {
  ...
  let dependerStableCount = 0;
  let dependerStableCountGoal = 0;
  const dependerStabilityEmitters = new Map<Emitter, () => void>();
  ...
  function stabilize () {
    ...
    if (
      isSignalDispatched ||
      activeTrackerCount === trackers.length
    ) {
      sendStableSignal();
      return;
    }
  ...
  function runAutoStabilize () {
    if (dependerStableCount === dependerStableCountGoal) {
      stabilize();
    }
  };
  ...
  let dispatchSignal = isOn ?
    _dispatchSignal :
    onStartDispatchSignal;
  ...
  const addDependentEmitter = autoStabilize ?
    (emitter: Emitter) => {
      dependerStableCountGoal++;
      dependerStabilityEmitters.set(
        emitter,
        emitter(() => {
          dependerStableCount++;
          runAutoStabilize();
  ...
  const _removeDependentEmitter = (emitter: Emitter) => {
    dependerStableCountGoal--;
    const terminator = dependerStabilityEmitters.get(emitter);
    if (terminator) {
      terminator();
    }
    dependerStabilityEmitters.delete(emitter);
  };
...
export function entangleOrbWithEmitter (
  orb: Orb, emitter: Emitter
) {
