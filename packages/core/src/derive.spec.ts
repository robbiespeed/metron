...
import { garbageCollect } from '@metron/test-utils/garbage-collect.js';
...
describe('core: Derive', () => {
  ...
  //   });
  it('should not break flow graph from eager GC', async function () {
    if (!garbageCollect) {
      this.skip();
    }
    const [a, setA] = state(1);
    let b = derive((r) => r(a));
    const c = derive((r) => r(b));
    const d = derive((r) => r(c));

    expect(d.unwrap()).to.equal(1);
    ...
    expect(d.unwrap()).to.equal(2);
  });
  it('should allow GC of inactive atoms after update', async function () {
    if (!garbageCollect) {
      this.skip();
    }
    let [a, setA] = state(1);
    ...
    const weakBOrb = b[ORB].weakRef;
    const weakAOrb = a[ORB].weakRef;
    ...
    a = undefined as any;
    setA = undefined as any;

    await garbageCollect();
    await garbageCollect();

    expect(weakAOrb.deref()).to.equal(undefined);
    expect(weakBOrb.deref()).to.equal(undefined);

    expect(d.unwrap()).to.equal(2);
  });
  it('should allow GC of inactive atoms without update', async function () {
    if (!garbageCollect) {
      this.skip();
    }
    let [a] = state(1);
    const b = derive((r) => r(a));

    expect(b.unwrap()).to.equal(1);

    const weakAOrb = a[ORB].weakRef;

    [a] = state(2);

    // 2 GC steps required due to finalization registry handling disposal of non-updatable state
    await garbageCollect();
    await garbageCollect();
  ...
  it('should allow disposed Orb to be derived from', async function () {
    if (!garbageCollect) {
      this.skip();
    }
    const [a] = state(1);
