...
function initElement(
  ...
  for (let i = 0; i < end; i++) {
    ...
    if (inst === 0) {
      ...
      const value =
        (inst & 0b1) === 0b1
          ? instanceValues[inst >> 16]
          : staticValues[inst >> 16];
        ...
        case 10: {
          if (isAtom(value)) {
            register(
              syncElementProp(
                node as Element,
                staticValues[(inst >> 4) & 0xfff],
                value
              )
            );
          } else {
            (node as any)[staticValues[(inst >> 4) & 0xfff]] = value;
          }
          break;
        }
        // Attribute
...
function createParams(
  ...
  for (let i = 0; i < end; i++) {
    ...
    switch (inst & 0b11) {
      // State Key
      case 0: {
        params[i] = state[staticValues[inst >> 2]];
        break;
      }
      // Param Key
      case 1: {
        params[i] = params[inst >> 14]?.[staticValues[(inst >> 2) & 0xfff]];
        break;
      }
      // State Fn
      case 2: {
        params[i] = staticValues[inst >> 2](state, context);
        break;
      }
      // Param Fn
      case 3: {
        params[i] = staticValues[(inst >> 2) & 0xfff](
          params[inst >> 14],
          context
        );
        break;
...
type SlotParent<TValue> = {
  [K in keyof TValue]: Slot<TValue[K]>;
};

type Slot<TValue> = _Slot & SlotParent<TValue>;
    ...
    switch (keySpecifier) {
      ...
      case 'prop': {
        if (slotPosition === -1) {
          instructions.push(
            10 |
              (getValuePosition(keyName) << 4) |
              (getValuePosition(value) << 16)
          );
        } else {
          instructions.push(
            11 | (getValuePosition(keyName) << 4) | (slotPosition << 16)
          );
        }
        continue;
      }
      case 'attr': {
        if (slotPosition >= 0) {
          instructions.push(
            13 | (getValuePosition(keyName) << 4) | (slotPosition << 16)
          );
      ...
      case 'toggle': {
        if (slotPosition >= 0) {
          instructions.push(
            15 | (getValuePosition(keyName) << 4) | (slotPosition << 16)
          );
        } else if (isAtom(value)) {
          instructions.push(
            14 |
  ...
  if (Array.isArray(children)) {
    ...
    for (let i = 0; i < childCount; i++) {
      ...
      if (slotPosition >= 0) {
        ...
        element.appendChild(
          buildIntrinsic(child, namespace, getValuePosition, instructions)
        );
    ...
    if (slotPosition >= 0) {
      ...
      const start = instructions.length;
      element.appendChild(
        buildIntrinsic(children, namespace, getValuePosition, instructions)
      );
...
*/
  ...
  const { instructions, paramInstructions, values } = blueprints;
  const params = createParams(state, context, values, paramInstructions);
  initElement(element, context, params, values, instructions);
  return element;
}
...
function createBlueprints(
  ...
  const slotParent = new _Slot(-1, paramInstructions, getValuePosition);
