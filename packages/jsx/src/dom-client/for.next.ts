...
class ElementChildrenSynchronizer<TValue> {
  ...
  #append(start: number) {
    const nodes = this.#indexedElements,
      cache = this.#cache,
    ...
    for (; start < length; start++) {
      ...
      const element = template(datum, context, (d) => {
        disposers.push(d);
      });
      nodes[start] = element;
  ...
  #fallback(start: number, prevValues: TValue[]) {
    ...
    for (i = start; i < size; i++) {
      ...
      if (unstableIndex === undefined) {
        ...
        const node = template(values[i]!, context, (dispose) => {
