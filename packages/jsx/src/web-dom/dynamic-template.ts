...
interface TemplateInit<TProps extends TemplateProps> {
  (
...
export function template<TProps extends TemplateProps>(
  init: TemplateInit<TProps>
): (props: TProps) => JsxRawNode {
  const templateNode = init(staticPropProxy, atomPropProxy);
  ...
  return createStaticComponent((props: TProps) => {
    ...
    if (descriptor !== undefined) {
      disposers = [];
      applyProps(element, props, disposers, descriptor);
    }
